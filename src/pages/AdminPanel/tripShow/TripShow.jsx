...
function TripShow({onClick, tripData}) {
  ...
  const updateData = () => {
    ...
    axiosApiInstance.put(`${process.env.REACT_APP_API_URL}/admin/trips/${trip_id}`, {
      name: trip_name,
      ...
      .then(res => {
        ...
        setData(trip);
        setLoading(false);
        window.location.href = `/admin/trips/${url_trip_id}`;
      })
      .catch(err => {
        console.log(err);
  ...
  const createData = () => {
    axiosApiInstance.post(`${process.env.REACT_APP_API_URL}/test`,
      {
        trip: {
          name: trip_name,
          location: trip_location,
          ...
          food: trip_food,
          activities: trip_activities,
          base_expenses: trip_base_expenses,
          total_per_day: trip_total_per_day,
          food_cuisine: trip_food_cuisine,
          season: trip_season,
          images: [trip_images],
          geometry: {
  ...
  const deleteData = () => {
    axiosApiInstance.delete(`${process.env.REACT_APP_API_URL}/admin/trips/${trip_id}`)
      .then(res => {
        console.log("Res", res);  
        setLoading(false);
        window.location.href = "/admin/trips";
  ...
  useEffect(() => {
    ...
    axiosApiInstance.get(`${process.env.REACT_APP_API_URL}/admin/trips/${url_trip_id}`)
      .then(res => {
        console.log("Res", res);
        setTrip(res.data.trip);
  ...
  return ( loading ? <h1>Loading...</h1> :
    <div>
      <div className="FormManagment">
        ...
        <Form>
          <Button type='submit' className='btn btn-danger' onClick={deleteData}>Delete</Button>
        </Form>
      </div>
    </div>
